...
class SubtitleGeneratorApp(ctk.CTk):
    ...
    def _build_layout(self) -> None:
        ...
        self.queue_box.grid(row=1, column=0, sticky="nsew", padx=16, pady=(0, 16))
        ...
        progress_area.grid(row=2, column=0, sticky="ew", padx=16, pady=(0, 16))
    ...
    def _start_processing(self) -> None:
        ...
        try:
            ...
            if not self.video_queue:
                messagebox.showinfo("Sem vídeos", "Selecione pelo menos um vídeo.")
                return
    ...
    def _burn_current_subtitle(self) -> None:
        """Burn the current SRT into a video copy using FFmpeg."""

        try:
            if self.result is None:
                return
            output = burn_subtitle(self.result.video_path, self.result.subtitle_path)
            messagebox.showinfo("Concluído", f"Vídeo legendado criado:\n{output}")
        except Exception as exc:
